...
import { increment, decrement, clearcart, removeCart } from "../CartSlice"
        ...
        <>
            {
                cart.map((cart) => (
                    <div className="flex justify-between items-center">
                        ...
                        <div>
                            <h2 className="font-semibold text-[17px] mb-3 text-end">${cart.pricing.price}</h2>
                            <div className="buttons flex gap-2 items-center justify-end">
                                <button onClick={() => dispatch(decrement(cart.id))} className="p-1 bg-black text-white rounded-3xl cursor-pointer"><FaMinus size={12} /> </button>
                                <p className="text-[16px] font-bold">{cart.qty}</p>
                                <button onClick={() => dispatch(increment(cart.id))} className="p-1 bg-black text-white rounded-3xl cursor-pointer"><FaPlus size={12} /> </button>
                            </div>
                        </div>
                    </div>
                ))
